 ...
 */
function stripComments(source: string): { clean: string; commentMap: Map<number, string> } {
 ...
 */
function extractMacros(source: string): string[] {
  const macros: string[] = [];
...
function parseStructs(source: string): StructDecl[] {
  const structs: StructDecl[] = [];

  // typedef struct [Tag] { ... } Alias;
  const typedefRe =
    /typedef\s+struct\s*(\w*)\s*\{/g;
  let m: RegExpExecArray | null;
  ...
  while ((m = plainRe.exec(source)) !== null) {
    ...
    if (structs.some((s) => s.name === m![1])) continue;
    const openBrace = m.index + m[0].length - 1;
    const closeBrace = findMatchingBrace(source, openBrace + 1);
    if (closeBrace < 0) continue;
    const body = source.slice(openBrace + 1, closeBrace).trim();
...
function parseEnums(source: string): EnumDecl[] {
  ...
  while ((m = plainRe.exec(source)) !== null) {
    if (enums.some((e) => e.name === m![1])) continue;
    const openBrace = m.index + m[0].length - 1;
    const closeBrace = findMatchingBrace(source, openBrace + 1);
    if (closeBrace < 0) continue;
    const body = source.slice(openBrace + 1, closeBrace).trim();
...
function parseTypedefs(source: string): TypedefDecl[] {
  const typedefs: TypedefDecl[] = [];
 ...
 */
export function inferRequiredIncludes(source: string): string[] {
  const needed: string[] = [];
 ...
 */
function parseFunctions(source: string, isC: boolean): FunctionDecl[] {
  ...
  for (const block of blocks) {
    ...
    if (body[closeParenIdx] !== ")") continue;

    // find matching open paren
    let pd = 1;
    let openParenIdx = closeParenIdx - 1;
    while (openParenIdx >= 0 && pd > 0) {
      if (body[openParenIdx] === ")") pd++;
      else if (body[openParenIdx] === "(") pd--;
      if (pd > 0) openParenIdx--;
    }
    if (pd !== 0) continue; // unbalanced
    ...
    if (!WORD.test(rawName) || skipNames.has(rawName)) continue;
...
function parseClasses(source: string): ClassDecl[] {
  ...
  while ((m = keywordRe.exec(source)) !== null) {
    ...
    if (charBefore.endsWith("enum")) continue;
    ...
    while (i < source.length) {
      ...
      if (ch === ";") break; // forward declaration
      basesRaw += ch;
      i++;
    }
...
function parseClassBody(body: string, isStruct: boolean): ClassSection[] {
  // Split into access sections
  const sections: ClassSection[] = [];

  // Default access: public for struct, private for class
  let currentAccess: "public" | "private" | "protected" = isStruct ? "public" : "private";
  let currentSection: ClassSection = {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (part === "public" || part === "private" || part === "protected") {
      currentAccess = part as "public" | "private" | "protected";
  ...
  return sections.filter(
    (s) => s.methods.length > 0 || s.fields.length > 0
  );
}

function parseClassSection(chunk: string, section: ClassSection): void {
  ...
  if (start < chunk.length) stmts.push(chunk.slice(start).trim());
  ...
  for (const stmt of stmts) {
    ...
    let openParenIdx = closeParenIdx - 1;
    while (openParenIdx >= 0 && pd > 0) {
      if (stmt[openParenIdx] === ")") pd++;
      else if (stmt[openParenIdx] === "(") pd--;
      if (pd > 0) openParenIdx--;
    }
    if (pd !== 0) continue;
...
function parseGlobalVars(source: string): string[] {
  const vars: string[] = [];
